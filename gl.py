...
class Model(object):
    def __init__(self, objName, textureName = None):
...
class Renderer(object):
    ...
    def render(self):
        ...
        if self.active_shader:
            #set camera position
            self.camPosition.y = glm.sin(glm.radians(self.camYangle)) * self.camRadius
            self.camPosition.x = glm.cos(glm.radians(self.camAngle)) * (self.camRadius - self.camPosition.y)
            self.camPosition.z = glm.sin(glm.radians(self.camAngle)) * (self.camRadius - self.camPosition.y)
